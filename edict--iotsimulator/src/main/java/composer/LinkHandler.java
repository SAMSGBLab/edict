...
//TODO check that creating broker is working correctly
public class LinkHandler {

	public void setConnections(CommonModel jmtModel, NgsiParser parser, HashMap<String, String> subtopicsClassSwitches) {
    	...
    	for (Topic topic : topics.values())
    		jmtModel.setConnected(jmtModel.getStationByName(topic.topicName + "_switch"), jmtModel.getStationByName("input"), true);
    	...
    	for (Actuator actuator : actuators.values())
    		for (String topic : actuator.subscribedTopics)
    			jmtModel.setConnected(jmtModel.getStationByName(topic + "_fork"), jmtModel.getStationByName(actuator.deviceName + "_outputQueue"), true);
    	
    	for (VirtualSensor virtualSensor : virtualSensors.values())
    		for (String topic : virtualSensor.subscribedTopics)
    			jmtModel.setConnected(jmtModel.getStationByName(topic + "_fork"), jmtModel.getStationByName(virtualSensor.deviceName + "_outputQueue"), true);
...
//    	2. topics class switch
    	for (Actuator actuator : actuators.values()) {
    		jmtModel.setConnected(jmtModel.getStationByName("output queue"), jmtModel.getStationByName(actuator.deviceName), true);
    		jmtModel.setConnected(jmtModel.getStationByName(actuator.deviceName), jmtModel.getStationByName("topics_class_switch"), true);
    	}
...
//    	3. class switch
    	for (VirtualSensor sensor : virtualSensors.values()) {
    		for (String topic : sensor.subscribedTopics) {
    			jmtModel.setConnected(jmtModel.getStationByName("output queue"), jmtModel.getStationByName(sensor.deviceName + "_processing"), true);
